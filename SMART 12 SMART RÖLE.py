...
stationary = adfuller(tt, autolag='AIC')
alfa = 0.01
if stationary[1] < alfa :
    print("ADF-P-value:",stationary[1])
    print("alfa anlamlılık düzeyi",alfa)
    print("0.01 anlamlılık düzeyinde birim kök olmadığı için seri durağandır,birim kök olduğu h0 hipotezi red edilir")
else:
    print("ADF-P-value:",stationary[1])
    print("alfa anlamlılık düzeyi",alfa)
    print("ho hipotezi reddedilemez , birim kök var , seri durağan değil")

meann = tt.rolling(window=4).mean()   
stdd = tt.rolling(window=4).std()

fig , ax = plt.subplots(figsize=(12,6))
ax.plot(tt,label="observation")
ax.plot(meann,label="movng avarage")
ax.plot(stdd,label="moving standard deviation")
...
history = model.fit(Xtrain,Ytrain,batch_size=41 ,epochs=5000,validation_data=(Xtest,Ytest))
...
Ypred_test = scaler.inverse_transform(Ypred_test)
Ypred_test = np.reshape(Ypred_test,len(Ypred_test))
Ypred_test = pd.Series(Ypred_test)
Yreel_test = tt1[44:]
#train predict
Ypred_train = model.predict(Xtrain)
Ypred_train = scaler.inverse_transform(Ypred_train)
...
model1.add(Dense(7))
model1.add(LSTM(96,activation="relu",return_sequences=True))
...
model1.fit(X,Y,batch_size=45 ,epochs=5000)
...
## Theil ' s U test 
from pysal.inequality.theil import Theil , TheilDSim , TheilD
theil=Theil(tt)
theilP = TheilDSim(Yreel,Ypred,permutations=999)
theilP.bg_pvalue
theil.T
